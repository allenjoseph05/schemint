...
# =============================================================================
# Agent Controller + Config Tests
# =============================================================================


class TestNotificationConfigFromSettings:
    """Tests for _notification_config_from_settings()."""

    @patch("schemint.config.get_settings")
    def test_returns_none_when_no_config(self, mock_settings):
        """Should return None when no webhook_url or github_token."""
        from schemint.drift.agent_controller import _notification_config_from_settings
        ...
        assert _notification_config_from_settings() is None

    @patch("schemint.config.get_settings")
    def test_returns_config_with_webhook(self, mock_settings):
        """Should return config when webhook_url is set."""
        from schemint.drift.agent_controller import _notification_config_from_settings

        mock_settings.return_value = MagicMock(
            webhook_url="https://example.com/hook",
            github_token=None,
            notification_webhook_headers='{"X-Key": "val"}',
            github_repo=None,
        )
        config = _notification_config_from_settings()
        assert config is not None
        assert config.webhook_url == "https://example.com/hook"
        assert config.webhook_headers == {"X-Key": "val"}

    @patch("schemint.config.get_settings")
    def test_returns_config_with_github(self, mock_settings):
        """Should return config when github_token is set."""
        from schemint.drift.agent_controller import _notification_config_from_settings
    ...
    @patch("schemint.config.get_settings")
    def test_invalid_json_headers_handled(self, mock_settings):
        """Invalid JSON headers should not crash."""
        from schemint.drift.agent_controller import _notification_config_from_settings

        mock_settings.return_value = MagicMock(
            webhook_url="https://example.com/hook",
            github_token=None,
            notification_webhook_headers="not json",
            github_repo=None,
        )
        config = _notification_config_from_settings()
        assert config is not None
