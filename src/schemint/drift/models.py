...
class PlanStep(BaseModel):
    ...
    generated_sql: AI-generated SQL from CopilotAgent (preferred over template SQL).
    rollback_sql: AI-generated rollback SQL from CopilotAgent.
    """
    ...
    generated_sql: str | None = None   # Pre-generated by CopilotAgent
    rollback_sql: str | None = None    # Pre-generated rollback by CopilotAgent
...
class ExecutionPlan(BaseModel):
    ...
    warnings contains critique notes from the optional plan critique pass.
    """
...
class ExecutionResult(BaseModel):
    ...
    metadata holds adapter-specific output: SQL that ran, issue URLs,
    commit status URLs, dry_run flags — available for audit and rollback.
    """
...
class RollbackReport(BaseModel):
    """Step-by-step results of a rollback execution.

    Generated by RollbackEngine when requires_rollback=True.
    Each step corresponds to the inverse of a previously executed step.
    """

    original_execution_id: str
    step_results: list[ExecutionResult] = Field(default_factory=list)
    overall_status: Literal["success", "partial_failure", "failed"] = "failed"
...
class ExecutionReport(BaseModel):
    ...
    rollback_report is populated after rollback is executed (if needed).
    """
...
# =============================================================================
# Agent Controller Models (Sprint 1)
# =============================================================================


class PredictedChange(BaseModel):
    """A single predicted change from applying migration SQL."""
...
class MigrationAlternative(BaseModel):
    """AI-generated safer alternative for a risky change."""

    original_change: str  # e.g. "DROP COLUMN users.email"
    safe_sql: str  # the generated safe SQL
    explanation: str
    risk_reduction: str  # e.g. "breaking -> safe"
    trade_off: str = ""


class RollbackScript(BaseModel):
    """AI-generated rollback SQL for the entire migration."""

    rollback_sql: str
    confidence: float = 0.0  # 0.0-1.0
    warnings: list[str] = Field(default_factory=list)
    is_complete: bool = False  # True if rollback covers all changes


class IntentAnalysis(BaseModel):
    """AI analysis of what the migration intends vs. what it does."""

    intent_matches: bool = True
    detected_intent: str = ""
    actual_behavior: str = ""
    suggestion: str = ""
    suggested_sql: str | None = None  # only if intent_matches=False


class SandboxWarning(BaseModel):
    """Warning from fast danger check."""

    pattern: str  # blocking_migration, destructive_change, etc.
...
class CopilotResult(BaseModel):
    """Full result from migration sandbox + co-pilot analysis."""

    sandbox_id: str
    migration_sql: str
    baseline_snapshot_id: str
    # Deterministic results
    predicted_changes: list[PredictedChange] = Field(default_factory=list)
    warnings: list[SandboxWarning] = Field(default_factory=list)
    safety_score: int = 100  # 0-100
    safety_grade: str = "A"  # A/B/C/D/F
    overall_risk: str = "safe"  # safe/needs_review/potentially_breaking/breaking
    # AI co-pilot results (empty if copilot not run)
    alternatives: list[MigrationAlternative] = Field(default_factory=list)
    rollback: RollbackScript | None = None
...
class DriftRunStatus(str):
    """Drift run status values.

    Uses str constants instead of Enum for Pydantic serialization simplicity.
    """
...
class MemoryContext(BaseModel):
    """Accumulated learning from past drift runs.

    Feeds into Phases 3 and 4 so the agent can avoid repeating
    previously-accepted findings and respect business rules.
    """

    accepted_findings: list[str] = Field(default_factory=list)
    business_rules: list[str] = Field(default_factory=list)
    schema_semantics: dict[str, str] = Field(default_factory=dict)
    table_change_frequency: dict[str, int] = Field(default_factory=dict)


class NotificationConfig(BaseModel):
    """Configuration for webhook and GitHub integrations."""
...
class StateTransition(BaseModel):
    """A single state transition in a drift run."""
...
class DriftRunResult(BaseModel):
    """Complete result of an autonomous drift pipeline run.

    Captures the full audit trail: every phase output, every state
    transition, and the final outcome.
    """

    run_id: str
    project_id: str
    status: str = DriftRunStatus.JUDGING
...
class RunTelemetry(BaseModel):
    """Per-run timing and cost telemetry for observability.

    Collected by AgentController and attached to DriftRunResult.
    ...
    # Phase durations keyed by phase name: "judging", "planning", "executing", "verifying"
    step_count: int = 0
    retry_count: int = 0
    copilot_enriched: bool = False  # True if CopilotAgent enriched any plan steps
