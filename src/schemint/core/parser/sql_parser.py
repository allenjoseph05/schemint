...
class SQLParser:
                    ...
                    if col_def_upper.startswith("PRIMARY KEY"):
                        ...
                        if column:
                            ...
                            if "REFERENCES" in col_def_upper:
                                fk = self._parse_inline_foreign_key(col_def_clean, column.name)
                                if fk:
                                    foreign_keys.append(fk)
    ...
    def _parse_column(self, col_def: str) -> Column | None:
        ...
        default = None
        for default_match in re.finditer(r"DEFAULT\s+([^,\s]+|'[^']*')", col_def, re.IGNORECASE):
            # ``GENERATED BY DEFAULT AS IDENTITY`` is identity syntax, not a
            # column DEFAULT expression.
            prefix = col_def[: default_match.start()].rstrip().upper()
    ...
    def _parse_inline_foreign_key(self, col_def: str, column: str) -> ForeignKey | None:
        """Parse a column-level ``REFERENCES`` constraint.

        PostgreSQL permits foreign keys directly on a column definition, for
        example ``user_id INT REFERENCES users(id)``.  These do not contain a
        ``FOREIGN KEY (...)`` clause, so the table-constraint parser cannot
        recognize them.
        """
        ref_match = re.search(
        ...
        name_match = re.search(r"CONSTRAINT\s+[`\"]?(\w+)[`\"]?", col_def, re.IGNORECASE)
        ...
        return ForeignKey(
            ...
            on_delete=delete_match.group(1).upper() if delete_match else None,
            on_update=update_match.group(1).upper() if update_match else None,
        )
