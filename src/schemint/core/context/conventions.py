        ...
        for table in schema.tables:
            ...
            if table.name.lower() in [t.lower() for t in deprecated["tables"]]:
                ...
                issues.append(Issue(
                    ...
                    description=dep_info.get("reason", "This table is deprecated") if dep_info else "This table is deprecated",
                    table_name=table.name,
                    impact="Using deprecated tables may cause issues in future versions",
                    fix_description=f"Use table '{dep_info.get('renamed_to')}' instead" if dep_info and dep_info.get("renamed_to") else "Remove usage of deprecated table",
                ))
            ...
            for col in table.columns:
                ...
                if col_key in [c.lower() for c in deprecated["columns"]]:
                    ...
                    issues.append(Issue(
                        ...
                        description=dep_info.get("reason", "This column is deprecated") if dep_info else "This column is deprecated",
                        table_name=table.name,
                        column_name=col.name,
                        impact="Using deprecated columns may cause issues in future versions",
                        fix_description=f"Use column '{new_name}' instead" if new_name else "Remove usage of deprecated column",
                    ))
